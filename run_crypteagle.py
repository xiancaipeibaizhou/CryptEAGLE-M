...
# ==========================================
# 【修改 1】导入你的超参和最新的 CryptEAGLE 模型
# ==========================================
from hparams_crypteagle import resolve_hparams
from model import CryptEAGLE  # 注意这里：改成导入 CryptEAGLE
...
def main():
    ...
    parser.add_argument('--variant', type=str, default='CryptEAGLE', 
                        choices=['CryptEAGLE', 'MILAN', 'WoGlobal', 'WoLocal', 'WoGating', 'WoEdgeAug', 'StandardTransformer'])
    ...
    # ==========================================
    # 【修改 3】实例化 CryptEAGLE 模型
    # ==========================================
    if args.variant == "CryptEAGLE": 
        model = CryptEAGLE(**model_kwargs).to(device)
    # 如果你还想保留以前的对照实验，把上面从 ablation_models 导包那段留着就行
    # elif args.variant == "MILAN": model = MILAN(**model_kwargs).to(device)
    else:
    ...
    for epoch in range(num_epochs):
        ...
        for step, batched_seq in enumerate(tqdm(train_loader, desc=f"Epoch {epoch+1}", leave=False)):
            ...
            # 【注意】：前向传播时传入 global_normal_prototype=None 触发对比推斥
            out = model(batched_seq, global_normal_prototype=None)
        ...
        val_auprc = average_precision_score(val_true_bin, val_attack_probs)
        
        log_line = f"Epoch {epoch+1:03d} | Loss: {total_loss/num_train_steps:.4f} | CL: {total_cl_loss/max(1, cl_loss_steps):.4f} | Val AUPRC: {val_auprc:.4f}"
        print(log_line)
        ...
        if val_auprc > best_val_score + h.get("MIN_DELTA", 0.0001): 
    ...
    print(f"  ✅ Done! Thresh: {best_th:.2f}, Test AUPRC: {test_auprc:.4f}, Test Macro F1: {metrics['F1 (Macro)']:.4f}, ASA: {metrics['ASA']:.4f}, FAR: {metrics['FAR']:.4f}")
    ...
    csv_file = "crypteagle_results.csv" 
